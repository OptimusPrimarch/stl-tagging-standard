...
- Replaced the copied `tsl.json` with a symlink to `tag-library.json` (needed one elevated PowerShell command to create) so there's exactly one file, tracked here, instead of two kept manually in sync.
